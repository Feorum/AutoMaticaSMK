...
STOP_SLOVA = {
    ...
    # Аббревиатуры-префиксы из длинных накладных:
    # «Изд.колб.вар.» → «изд», «колб», «вар»
    # «Колб.тер.об./мп.к-са лив.» → «тер», «мп», «лив», «кол»
    "изд", "колб", "вар", "тер", "мп", "лив", "кол", "кб",
    # «Полуколб.из.» → «полуколб», «из»
    "полуколб", "из",
    # «мяс.» (мясная, мясной) — слишком общее, у сотни товаров
    "мяс",
    # Описание оформления, которое уже извлекает oformlenie_slovar,
    # но иногда остаётся в тексте частично:
    "модифицированная", "модифицированный", "модифицированной",
    "атмосфера", "атмосфере",
    "групповая", "упаковка",
    # Паштетный/паштетная — категорийное, есть в KAT_PREFIXY, но
    # «пашт» (сокращение в накладных) нужно отдельно
    "пашт",
    # «натур» (натуральная оболочка) — встречается как сокращение
    "натур",
}
...
def clean_and_stem(text: str) -> str:
    ...
    if not text:
        return ""
    # Убираем кавычки (в накладных «Название»): мешают token_set_ratio
...
def obrabotat_zadanie(path: str, tovary: list[dict]) -> bool:
    ...
    # plan_idx: set занятых позиций → защита от дублей при нескольких
    # строках задания, которые нашли один и тот же товар.
    plan_idx: set[int] = set()

    for name, oform, massa, kol in pozicii:

        # Защита от вырожденного запроса: если после очистки остаётся
        # ≤ 1 слова — запрос слишком короткий, совпадёт со всем подряд.
        chistoe = clean_and_stem(name)
        if len(chistoe.split()) <= 1:
            print(f"  ! Запрос слишком короткий после очистки: {name!r} → {chistoe!r}")
            print(f"    Позиция пропущена — уточните название вручную.")
        ...
        # Дедупликация: если этот товар уже в плане — суммируем количество,
        # не добавляем повторную строку.
        if idx in plan_idx:
            for p in plan:
                if p["idx"] == idx:
                    p["kol"] += int(kol)
                    print(f"  ~ Дубль: стр {idx+1} {t['naimenovanie']!r} — "
                          f"суммируем кол-во (+{kol}), итого {p['kol']}")
                    break
        else:
            plan_idx.add(idx)
                ...
                if 1 <= num <= len(v["varianty"]):
                    ...
                    if cidx in plan_idx:
                        for p in plan:
                            if p["idx"] == cidx:
                                p["kol"] += v["kol"]
                                print(f"    → дубль: стр {cidx+1} {format_tovar(tv)} "
                                      f"суммируем (+{v['kol']}), итого {p['kol']}")
                                break
                    else:
                        plan_idx.add(cidx)
