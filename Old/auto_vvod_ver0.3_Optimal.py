# -*- coding: utf-8 -*-
"""
auto_vvod.0.56.py — помощник для ввода количества продукции в старую DOS/Oracle-программу.
...
# Это важно: у onefile-exe __file__ указывает во временную папку,
# а нам нужна папка рядом с самим exe (на флешке).
if getattr(sys, "frozen", False):
    BASE_DIR = os.path.dirname(os.path.abspath(sys.executable))
else:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Папка со справочниками продукции (.txt).
SPISKI_DIR = os.path.join(BASE_DIR, "../spiski")

# Папка с файлами-заданиями (.txt).
...
# Сортировать ввод по номеру строки справочника (минимум перемещений).
# True — курсор идёт в одну сторону без лишних разворотов.
...
def main():
    ...
    # Диагностика: где скрипт ищет папки.
    print(f"Папка справочников: {SPISKI_DIR}")
    print(f"Папка заданий:    {ZADANIYA_DIR}")
    print()
    ...
    if not spravochniki:
        ...
        print("Создайте папку 'spiski' РЯДОМ с exe и положите туда .txt-справочник.")
        return
    ...
    # ОПТИМИЗАЦИЯ МАРШРУТА: сортируем по номеру строки, чтобы курсор
    # шёл сверху вниз без лишних разворотов (минимум нажатий).
...
def _uderzhat_okno():
    """Чтобы окно exe не закрывалось мгновенно при двойном клике."""
    try:
...
if __name__ == "__main__":
    try:
        ...
        # Любая неожиданная ошибка — покажем, а не закроем окно молча.
        import traceback
        print("\n!!! ОШИБКА !!!")
        traceback.print_exc()
    finally:
