...
from Old.oformlenie_slovar import (
...
def naiti_kandidatov(
    ...
    Оформление и масса часто вписаны ВНУТРЬ названия заказа (напр.
    "Докторская кат А П/а короб 650"). Если явно не переданы — извлекаем
    их из строки и используем для различения дубликатов-названий."""
...
def obrabotat_zadanie(path: str, tovary: list[dict]) -> bool:
    ...
    plan: list[dict] = []
    ne_naideno: list  = []
    neodnoznachno: list = []   # неразличимые дубликаты — пропускаем, спросим оператора
    for name, oform, massa, kol in pozicii:
        ...
        if ambig:
            # Несколько одинаково подходящих позиций — НЕ вводим автоматически.
            varianty = [(k[0] + 1, k[1]) for k in kand
                        if k[2] > score - 0.05][:4]
            neodnoznachno.append((name, oform, massa, kol, varianty))
            continue
        plan.append({
            "idx":   idx,
            "t":     t,
    ...
    if neodnoznachno:
        print("\n  ТРЕБУЕТСЯ РУЧНОЙ ВЫБОР (пропущены — несколько подходящих):")
        for name, oform, massa, kol, varianty in neodnoznachno:
            uточн = " | ".join(x for x in [oform, massa] if x)
            print(f"    {name}{' | ' + uточн if uточн else ''} = {kol}")
            for poz, tv in varianty:
                print(f"        ? стр {poz}: {format_tovar(tv)}  (ост {tv['ostatok']})")
        print("    -> Введите эти позиции вручную в программе "
              "(заказ уточнить у склада).")
